<?php
    ...
    echo '<div class="block_text-pics d-flex align-items-center my-12">';
        ...
        echo '<div class="block_pics col-md-6 " >';
            if( $images ): ?>
                echo' <div id="masonry-grid" class="grid">';'
                    <?php foreach( $images as $image ):
                        echo '<div class="grid-item" '.$dataAosImage.'>';
                            echo '<a href="'.$image['url'].'" data-lightbox="Jonesy" data-title="Jonesy Agency - studio Paris 11" >';
                                echo '<img src="'.esc_url($image['sizes']['thumbnail']).'" alt="Thumbnail of '.esc_url($image['alt']).'" />';
                            echo '</a>';
                        echo '</div>';
                    endforeach;
                echo '</div>';
